...
static void select_btn_click_event_cb(lv_event_t * e){
    ...
    if(device_state->tomato_time_state == ON){
        device_state->tomato_time_state = OFF;
        is_stop = true;
        lv_label_set_text(btn_label,"恢复");
    }else{
...
static void refresh_timer_cb_func(lv_timer_t * timer)
{
    ...
    if(is_symbol){//闪烁实现
        is_symbol = false;
        lv_label_set_text_fmt(time_label,"%s",get_time_str(device_state->tomato_learn_remaining_time));
    }else{
        is_symbol = true;
        lv_label_set_text_fmt(time_label,"%s",get_time_str_nosymbol(device_state->tomato_learn_remaining_time));
    }
...
static lv_obj_t * init_title_view(lv_obj_t *parent){
    ...
    lv_label_set_text(title,"番茄时钟-学习中");
...
static lv_obj_t * init_info_view(lv_obj_t *parent){
    ...
    device_state_t* device_state = get_device_state();
    device_state->tomato_learn_remaining_time = device_state->tomato_learn_time;
    lv_label_set_text_fmt(time,"%s",get_time_str(device_state->tomato_learn_remaining_time));    
...
static lv_obj_t * init_select_btn(lv_obj_t *parent){
    ...
    btn_label = label;
    obj_font_set(label,FONT_TYPE_CN, FONT_SIZE_TEXT_1);
...
void init_page_tomato_learn(void)
{
    ...
    start_play_audio_async(GET_MUSIC_PATH("audio_start.wav"));
}